/**
 * Generador lista DISTRIBUCIÓN INTERIOR desde simulación verificada
 * Hojas: AVANTI MASAS | AVANTI PASTAS Y SALSAS | PASTAMANIA
 * Columna F = precio_neto (costo distribuidor s/IVA), G = precio c/IVA, H = PVP sugerido al público
 * Precio interior = precio comercio / 1.25 (margen distribuidor ~62.5% sobre PVP)
 */
...
const DBROWN = 'FF4E342E'   // marrón oscuro para el banner de interior/distribución
...
function buildSheet(
  ...
  bannerCell.value     = `DISTRIBUCIÓN INTERIOR${listaNombre}  |  Vigencia: ${fechaVig}`
...
export async function generarInteriorXlsx({ items, vigencia, nombre }: Params): Promise<Buffer> {
